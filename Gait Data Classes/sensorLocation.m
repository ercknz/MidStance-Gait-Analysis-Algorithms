classdef sensorLocation < handle
   ...
   methods
       ...
       function obj = filterData(obj, dataToFilter)
           if ~isempty([obj.filtA, obj.filtB])
               for i = 1:3
                   if ~isempty(obj.acc) && (strcmp('acc',dataToFilter) || strcmp('all',dataToFilter))
                       obj.fPreAcc(:,i) = filtfilt(obj.filtB, obj.filtA, obj.preAcc(:,i));
                       obj.fAcc(:,i) = filtfilt(obj.filtB, obj.filtA, obj.acc(:,i));
                   end
                   if ~isempty(obj.gyro) && (strcmp('gyro',dataToFilter) || strcmp('all',dataToFilter))
                       obj.fPreGyro(:,i) = filtfilt(obj.filtB, obj.filtA, obj.preGyro(:,i));
                       obj.fGyro(:,i) = filtfilt(obj.filtB, obj.filtA, obj.gyro(:,i));
                   end
                   if ~isempty(obj.mag) && (strcmp('mag',dataToFilter) || strcmp('all',dataToFilter))
